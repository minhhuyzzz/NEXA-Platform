...
const OrgUsers = () => {
  ...
  // DỮ LIỆU MẪU (Không dùng biểu đồ để tránh lỗi)
  const [users, setUsers] = useState([
    ...
    { id: 5, name: 'Trần Thu Hà', code: 'SV-2024155', email: 'ha.tran@design.com', role: 'student', score: 910, status: 'active', dept: 'Design' },
  ]);
  ...
  return (
    <div className="flex h-screen bg-[#f8fafc] font-sans text-slate-900 overflow-hidden text-left">
      ...
      {/* 1. SIDEBAR (Tích hợp sẵn để menu hoạt động trơn tru) */}
      ...
      {/* 2. NỘI DUNG CHÍNH (GIAO DIỆN TABLE CAO CẤP) */}
      <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-[#f8fafc]">
        ...
        <header className="px-8 py-6 border-b border-slate-100 bg-white/80 backdrop-blur-md sticky top-0 z-10 flex justify-between items-center shrink-0">
          <div>
            ...
            <p className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mt-1">Quản lý thành viên & Cấp phát chứng chỉ</p>
          </div>
          <div className="flex gap-3">
            <button className="flex items-center gap-2 px-5 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-slate-50 transition-all">
