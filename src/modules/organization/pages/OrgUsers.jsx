...
import { 
  ...
  CheckCircle2, Clock, MoreVertical, ShieldCheck, UserCheck 
} from 'lucide-react';
...
const OrgUsers = () => {
  ...
  const [users, setUsers] = useState([
    { id: 1, name: 'Nguyễn Thị Bích', code: 'SV-2024001', email: 'bich.nguyen@edu.vn', role: 'student', score: 850, status: 'completed', dept: 'Kế toán' },
    { id: 2, name: 'Lê Văn Cường', code: 'GV-005', email: 'cuong.le@techcorp.com', role: 'staff', score: 920, status: 'active', dept: 'IT Soft' },
    { id: 3, name: 'Phạm Minh Dũng', code: 'SV-2024088', email: 'dung.pm@iuh.edu.vn', role: 'student', score: 780, status: 'pending', dept: 'Marketing' },
    { id: 4, name: 'Hoàng Anh Tuấn', code: 'SV-2024102', email: 'tuan.ha@gmail.com', role: 'student', score: 885, status: 'completed', dept: 'Sales' },
  ]);
  ...
  useEffect(() => {
    ...
    if (savedUser.fullName) {
      setUsers(prev => [{
        ...
        code: savedUser.idCode || 'ADMIN-01',
  ...
  return (
    <div className="flex h-screen bg-[#f8fafc] font-sans text-slate-900 overflow-hidden text-left">
      ...
      {/* 1. SIDEBAR (Giữ nguyên style chuẩn) */}
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <header className="px-8 py-6 border-b border-slate-100 bg-white/80 backdrop-blur-md sticky top-0 z-10 flex justify-between items-center shrink-0">
          <div>
            ...
            <p className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mt-1">Quản lý toàn bộ thành viên tổ chức</p>
          </div>
          <div className="flex gap-3">
            <button className="flex items-center gap-2 px-5 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-slate-50 hover:border-slate-300 transition-all">
            ...
            <div className="flex items-center gap-4 bg-white p-2 rounded-2xl border border-slate-100 shadow-sm">
              <div className="flex-1 relative">
                ...
                <input 
                  ...
                  className="w-full pl-11 pr-4 py-3 bg-transparent outline-none font-medium text-sm text-slate-700 placeholder:text-slate-400"
                  value={searchTerm}
              ...
              <div className="flex gap-2 pr-2">
                ...
                <FilterBtn label="Hoàn thành" active={filterStatus === 'completed'} onClick={() => setFilterStatus('completed')} />
                <FilterBtn label="Đang thi" active={filterStatus === 'active'} onClick={() => setFilterStatus('active')} />
              </div>
            </div>
            ...
            <div className="bg-white rounded-[24px] border border-slate-100 shadow-sm overflow-hidden">
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50/50 border-b border-slate-100">
                  <tr>
                    <th className="px-8 py-5 text-[10px] font-[1000] text-slate-400 uppercase tracking-[0.2em]">Thông tin</th>
                    ...
                    <tr key={user.id} className="hover:bg-blue-50/30 transition-colors group cursor-pointer">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-[#3b66f5] font-black shadow-sm group-hover:scale-110 transition-transform bg-gradient-to-br from-blue-50 to-white">
                            {user.name.charAt(0)}
                          </div>
                          <div>
                            <p className="font-[1000] text-sm text-slate-800 uppercase tracking-tight group-hover:text-[#3b66f5] transition-colors">{user.name}</p>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{user.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <span className="px-3 py-1.5 rounded-lg bg-slate-50 text-slate-600 text-[10px] font-bold uppercase tracking-wide border border-slate-100">
                      ...
                      <td className="px-8 py-5 text-right">
                        <button className="p-2 hover:bg-slate-100 rounded-xl text-slate-400 hover:text-[#3b66f5] transition-all">
                    ...
                    <tr>
                      <td colSpan="5" className="px-8 py-12 text-center">
                        <div className="flex flex-col items-center justify-center text-slate-300">
                          ...
                          <p className="text-sm font-bold uppercase tracking-wider">Không tìm thấy kết quả nào</p>
                        </div>
                      </td>
                    </tr>
                  )}
...
/* --- CÁC COMPONENT CON (Sub-Components) --- */

const MenuBtn = ({ icon, label, active, onClick }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl transition-all font-bold text-sm ${active ? 'bg-blue-50 text-[#3b66f5] shadow-sm' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-600'}`}>
...
const KPICard = ({ title, value, icon, bg }) => (
  <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
    ...
    <div>
      ...
      <p className="text-2xl font-[1000] text-slate-800 tracking-tighter">{value}</p>
    </div>
  </div>
);
